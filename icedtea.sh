...
if test x${ICEDTEA_WITH_WERROR} = "xyes"; then
    WERROR_OPTION="--enable-Werror";
else
    WERROR_OPTION="--disable-Werror";
fi

if test x${ICEDTEA_WITH_CURVES} != "x"; then
...
CONFIG_OPTS="--prefix=${INSTALLATION_DIR} --mandir=${INSTALLATION_DIR}/man \
    ...
    ${CACAO_OPTION} ${CACAO_ZIP_OPTION} ${SHARK_OPTION} ${VISUALVM_OPTION} ${PULSEAUDIO_OPTION} ${WERROR_OPTION} \
