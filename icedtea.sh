...
CONFIG_OPTS="--with-parallel-jobs=${PARALLEL_JOBS} --with-libgcj-jar=${GCJ_JDK_INSTALL}/jre/lib/rt.jar \
    ...
    --disable-docs ${CACAO_OPTION} ${CACAO_ZIP_OPTION} ${SHARK_OPTION} \
    --with-icedtea-home=${ICEDTEA_INSTALL} ${OPTS} --with-java=${GCJ_JDK_INSTALL}/bin/java \
    --with-jar=${GCJ_JDK_INSTALL}/bin/jar --with-javah={GCJ_JDK_INSTALL}/bin/javah"
